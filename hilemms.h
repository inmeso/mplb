...
//scheme: which schemes to use for implementing such as finite difference scheme.
...
void DefineBlockBoundary(int blockIndex, int componentID, BoundarySurface surface, 
                         BoundaryType type, std::vector<VariableTypes> MacorVarsComp, std::vector<Real> valuesMacroVarsComp);
...
//valueMacroVarsComp: specified value for the boundary condition for the macro vars which are defined using MacroVarsComp.

void CalBlockCoordinates(const int blockIndex, Real* blockStartPos, Real meshsize);
//blockIndex: Block Index
//blockStartPos: Starting position of the block in x, y, z directions to find coordinates. 
//meshSize: Size of mesh (Assuming a constant in all 3 directions for stream collision scheme).


void KerSetCoordinates(const Real* coordX, const Real* coordY, const int* idx, Real* coordinates);
void KerSetCoordinates3D(const Real* coordX, const Real* coordY, const Real* coordZ, const int* idx, Real* coordinates);
...
//This function was defined in setup_comput_domain and has been declared here as we are not using the preprocessor code separately.

void DefineIntialCond(int blockIndex, int componentId, std::vector<Real> initialMacroValues);
//blockIndex: Block Id.
//componentId: Id of the component.
//initialMacroValues: Initial values of the macroscopic variables for a given component in a particluar block.
